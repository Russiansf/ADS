...
def start_gm(ads_id, first_start): #,  sec=10
    ...
    if driver_init(ads_id):
        ...
        # WebDriverWait(driver, 10).until(ec.visibility_of_element_located((By.XPATH, xpath_change_proxy))).click()
        # print('change proxy 0000')
        if first_start:
            close_other_handles()
            time.sleep(1)
...
def try_gm():
    ac(driver).send_keys(Keys.END).perform()
    # ac(driver).scroll_to_element(xpath_gm).perform()
